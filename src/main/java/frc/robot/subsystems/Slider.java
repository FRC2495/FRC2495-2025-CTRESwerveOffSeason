...
/**
 * The {@code Slider} class contains fields and methods pertaining to the function of the slider.
 */
public class Slider extends SubsystemBase implements ISlider {
	...
	WPI_TalonSRX slider; 
	//BaseMotorController slider_follower;
	...
	public Slider(WPI_TalonSRX slider_in /*BaseMotorController slider_follower_in,*/ /*Robot robot_in*/) {
		...
		slider.configReverseLimitSwitchSource(LimitSwitchSource.FeedbackConnector, LimitSwitchNormal.NormallyOpen, TALON_TIMEOUT_MS);
		slider.overrideLimitSwitchesEnable(true);
		...
		//sliderfollower.setStatusFramePeriod(StatusFrame.Status_1_General, 255, TALON_TIMEOUT_MS);
		//sliderFollower.setStatusFramePeriod(StatusFrame.Status_2_Feedback0, 255, TALON_TIMEOUT_MS);

		setPIDParameters();
		
		// use slot 0 for closed-looping
 		//slider.selectProfileSlot(SLOT_0, PRIMARY_PID_LOOP);
	...
	public boolean tripleCheckMove() {
		if (isMoving) {
			...
			double error = slider.getClosedLoopError(PRIMARY_PID_LOOP);
			...
			if (isOnTarget) { // if we are on target in this iteration 
				...
				if (onTargetCount > 0) { // even though we were on target at least once during a previous iteration
					...
					System.out.println("Triple-check failed (slider moving).");
				} else {
			...
			if (!isMoving) {
				System.out.println("You have reached the target (slider moving).");
	...
	public int getEncoderVelocity() {
		return (int) (slider.getSelectedSensorVelocity(PRIMARY_PID_LOOP));
	}
	...
	private void setPIDParameters() {		
		slider.configAllowableClosedloopError(SLOT_0, TALON_TICK_THRESH, TALON_TIMEOUT_MS);
		...
		slider.config_kP(SLOT_0, MOVE_PROPORTIONAL_GAIN, TALON_TIMEOUT_MS);
		slider.config_kI(SLOT_0, MOVE_INTEGRAL_GAIN, TALON_TIMEOUT_MS);
		slider.config_kD(SLOT_0, MOVE_DERIVATIVE_GAIN, TALON_TIMEOUT_MS);
		slider.config_kF(SLOT_0, 0, TALON_TIMEOUT_MS);
	}
	
	// NOTE THAT THIS METHOD WILL IMPACT BOTH OPEN AND CLOSED LOOP MODES
	public void setNominalAndPeakOutputs(double peakOutput)
	{
		slider.configPeakOutputForward(peakOutput, TALON_TIMEOUT_MS);
		slider.configPeakOutputReverse(-peakOutput, TALON_TIMEOUT_MS);
		
		slider.configNominalOutputForward(0, TALON_TIMEOUT_MS);
		slider.configNominalOutputReverse(0, TALON_TIMEOUT_MS);
	}
